...
//parameter1: 1.控制的對象 2.時間(秒) 3.起始位置(控制對象的原始狀態) 4.結束位置(動畫結束後的狀態)5.動畫效果
...
allSelects.forEach((select) => {
  select.addEventListener('change', (e) => {
    setGPA();
    changeColor(e.target); // e.target = select標籤
  });
});
...
function setGPA() {
  ...
  // 選擇所有具有 .class-credit 類別的 HTML 元素，通常是用來表示學分的輸入欄位
  let credits = document.querySelectorAll('.class-credit');
  // 選擇所有的 select 標籤元素，通常用於選擇每個課程的成績
  let selects = document.querySelectorAll('select');
  // 初始化 sum（GPA 分子）變量，用於存儲加權後的成績之和
  let sum = 0;
  // 初始化 creditSum（GPA 分母）變量，用於存儲學分之和
  let creditSum = 0;
  ...
  for (let i = 0; i < credits.length; i++) {
    ...
    if (!isNaN(credits[i].valueAsNumber)) {
      // 將每個學分輸入欄位的值（數字）累加到 creditSum 變量中
...
addButton.addEventListener('click', () => {
  ...
  newInput1.classList.add('class-number');
